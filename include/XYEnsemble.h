...
public:
  ...
  bool GrowOneChain(int m_samples, int j, int numnodes, int a);
	// growth chain
 bool GrowmChain(int k, int f, int m_samples);
  ...
  void GetGoodPoints( CXYVector<float>& prvnode, CXYMatrix<float>& kMSamplesPoints,vector<int>& GoodPointInd, vector<int>& NoCollisionPointInd, int j);

  void CreateNewPointTree(int k, int m_samples, CXYVector<int> vectorofI );
